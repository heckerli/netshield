...
public:
	...
	INT32_T resetState()
    {
        if(parseOnly->count == 0 || logFile != NULL)
        {
            ...
            while(csalgoColumnMatchedRuleVectorIt != csalgoColumnMatchedRuleVector.end())
            {
                if((*csalgoColumnMatchedRuleVectorIt)->size() > 0)
                {
...
// protected:
    ...
    inline INT32_T matchSimpleFieldLength(Field * field, IntRangeMatcher<Rule> * lengthMatcher)
    {
        ...
        lengthMatcher->match(field->dataEnd - field->dataBegin);
        
        if(field->parsingState == FINISHED)
        {
            vector<Rule>::const_iterator first, last;
            lengthMatcher->getGtCurrentState(&first, &last);
            
            vector<Rule>::const_iterator it = first;
            while(it != last)
            {
                // cout << (*it).data << "\n";
                                  ...
                                  IntRangeMatcher<UINT32_T> * lengthGroupMatcher)
    {
        ...
        if((first->matchingState & STRING_MATCHED) != 0 && *dictKeyData != NULL)
        {
            if(second->parsingState == INPROCESS)
            {
                ...
                if((*dictKeyData)->lengthGroup >= 0)
                {
                    lengthGroupMatcher->match(second->dataEnd - second->dataBegin);
                }
            }
            else if(second->parsingState == FINISHED)
            {
                ...
                if((*dictKeyData)->lengthGroup >= 0)
                {
                    ...
                    vector<UINT32_T>::const_iterator lengthDataVectorIt = first;
                    while(lengthDataVectorIt != last)
                    {
                        vector<Rule>::const_iterator lengthRuleVectorIt = ((*dictKeyData)->lengthRuleVector[*lengthDataVectorIt])->begin();
                        while(lengthRuleVectorIt != ((*dictKeyData)->lengthRuleVector[*lengthDataVectorIt])->end())
                        {
