...
VOID_T displayStat()
{
    ...
    if(protocol == PROTOCOL_HTTP || protocol == PROTOCOL_DCE_RPC)
    {
...
int main(int argc, char ** argv)
{
    ...
    */
    ...
    /* Parsers of protocol DCERPC and DNS is under development and may not run properly. */
    proto         = arg_str0("p", NULL, "<protocol>", "<HTTP, DCERPC, DNS>, default HTTP.");
    ...
    else if(proto->count == 1)
    {
        ...
        else if(stricmp(proto->sval[0], "DCERPC") == 0)
        {
    ...
    {
		if(protocol == PROTOCOL_HTTP && parseOnly->count + tcpReassembly->count + udpReassembly->count + seqMatch->count + help->count == 0)
        {
...
exit:
    ...
    if(seqMatch->count > 0 && protocol == PROTOCOL_HTTP)
    {
        HTTPAnalyzerSeqInit();
    }
