...
// generated by declaring a CaseType in pac_parse.yy, it passes
// the index expression and cases as seen in the parser

// if i understand this correctly, it generates a CaseType first,
// then uses CaseField to produce the actual code to be used
// in the result

// there seem to be multiple places that generate switch statements (3?)
// and i'm not sure which is which although i believe the one that's
// commented accordingly is the one mainly used, so the difference between
// parse code and public declarations may need to be clarified

// functions for case types
...
// AddField is from Type, so it's just some generic field method
...
// returns string, pointer of data type
...
// default value of case type, e.g. int = 0, etc.
string CaseType::DefaultValue() const
	{
	return ValueType()->DefaultValue();
	}

// this gets the cases ready
void CaseType::Prepare(Env* env, int flags)
    ...
    // despite sorting it above and whatever else, it doesn't want a
    // default case here because it will be generated with an exception
    // as it's body when the actual switch statement is printed
    // see: GenPubDecls
	if ( default_case )
		{
...
// next 3 functions are just standard code
...
// clean up code, self explanatory
...
void CaseType::DoGenParseCode(Output* out_cc, Env* env,
	...
	if (! has_default_case && FLAGS_no_error_detection == false)
		{
...
// i believe the following have to do with user declarations
// such as byte order and stuff
...
// This generates the list of "case xyz:" lines that will be used
// when the code is printed.
void GenCaseStr(ExprList *index_list, Output *out_cc, Env *env)
	...
	if ( index_list )
		...
		foreach(i, ExprList, index_list)
            ...
            // won't accept constant expressions?
...
// This prints the whole switch statement.
...
// i can't figure out what this is supposed to be for
