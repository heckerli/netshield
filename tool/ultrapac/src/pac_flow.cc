...
void FlowDecl::GenProcessFunc(Output *out_h, Output *out_cc)
	...
	out_h->println("int %s;", proto.c_str());

	out_cc->println("int %s::%s", class_name().c_str(), proto.c_str());
	out_cc->inc_indent();
	out_cc->println("{");
	...
	out_cc->inc_indent();
	out_cc->println("{");
	out_cc->println("DEBUG_MSG(\"binpac exception: %%s\\n\", e.c_msg());");
...
void FlowDecl::GenCodeDatagram(Output *out_cc)
	...
	out_cc->println("return %s;", env_->LValue(unit_datatype->size_var()));
	}
