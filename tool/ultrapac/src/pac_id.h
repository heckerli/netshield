...
public:
//protected:
	string name;			//the yytext() in protocol specification, or generated string for anonymous ID
	bool anonymous_id_;	//true if it is an anonymous id
	char *locname;		//inputfilename:linenumber:name
	friend class ID_ptr_cmp;

public:
	static ID *NewAnonymousID(const string &prefix);
private:
	static int anonymous_id_seq;	//the count for anonymous id
};
