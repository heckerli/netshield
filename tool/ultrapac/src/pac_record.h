...
//private:
public:
	Field *parsing_dataptr_var_field_;	//initialized to 0
	RecordFieldList* record_fields_;	//list of field of current record
};
...
//protected:
public:
	RecordType* record_type_;	//initialized to 0
	RecordField* prev_;		//pointer to field before this field within the same record
	RecordField* next_;		//pointer to field after this field
	bool boundary_checked_;//initialized to false
	int static_offset_;
	int parsing_state_seq_;

	DataPtr* begin_of_field_dataptr;	//initialized to 0
	DataPtr* end_of_field_dataptr;	//same
	char* field_size_expr;			//same
	char* field_offset_expr;			//same
	ID* end_of_field_dataptr_var;	//same
