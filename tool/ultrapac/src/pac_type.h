...
//protected:
public:
	TypeType tot_;		//the type of Type. StringType, RecordType, CaseType, etc. represents different child class
	const TypeDecl *type_decl_;	//initialized to 0. If not 0, points to the TypeDecl that contains current type 
	bool declared_as_type_;		//initialized to false, set to true if this Type is part of TypeDecl
	const ID *type_decl_id_;		//id of current declaration, which is the larger structure that contains current type specification
	Env *env_;			//initialized to 0

	const ID *value_var_;		//0 for RecordType
	bool anonymous_value_var_;	// initialized to false

	string data_id_str_;
	int value_var_type_;	//initialized to MEMBER_VAR
	Field *size_var_field_;	//initialized to 0
	char *size_expr_;		//initialized to 0
	bool boundary_checked_;	//initialized to false
	string lvalue_;
	FieldList *fields_;		//Initialized to an empty field list. Stores all fields within this type

	bool incremental_input_;	//initialized to false
	bool incremental_parsing_;	//same
	bool buffer_input_;		//same
	...
	// Attributes
	AttrList *attrs_;	//new an empty list. The &let statement together forms one attr. It's one element in attrs_

	Expr *attr_byteorder_expr_;	//if not NULL, is the byte order attribute
	ExprList *attr_checks_;	//new an empty list
	bool attr_chunked_;
	bool attr_exportsourcedata_;
	Expr *attr_if_expr_;
	Expr *attr_length_expr_;		//if not NULL, is the length expression of the type
	FieldList *attr_letfields_;	//all let fields in &let statement.
