...
extern bool FLAGS_no_error_detection;
extern bool FLAGS_not_to_save_array_elements;
extern bool FLAGS_not_to_save_strings;
...
void TypeDecl::GenCode(Output* out_h, Output* out_cc)
	...
	AddBaseClass(&base_classes);	//add base class declaration
	...
	bool first = true;
	foreach(i, vector<string>, &base_classes)		//print out declaration for base class
		{
		if ( first )
			{
	...
	GenConstructorFunc(out_h, out_cc);		//print out the class' construction and destruction function
	GenDestructorFunc(out_h, out_cc);

	if ( type_->attr_length_expr() )
		GenInitialBufferLengthFunc(out_h, out_cc);	//add a function "initialBufferLength", to return a int value

	/*parse function generation, key function*/
...
void TypeDecl::GenConstructorFunc(Output* out_h, Output* out_cc)
	{
	string params_str = ParamDecls(params_);	//get the string of parameter declaration

	string proto = 
		strfmt("%s(%s)", class_name().c_str(), params_str.c_str());

	out_h->println("%s;", proto.c_str());		//output name and parameter in .h and .cc files
	...
	type_->GenInitCode(out_cc, env_);	//output a series of "member variable xxx= 0;"
	GenInitCode(out_cc);	//do nothing
...
void TypeDecl::GenDestructorFunc(Output* out_h, Output* out_cc)
	...
	out_cc->println("%s::%s", class_name().c_str(), proto.c_str());	//output function head in .h and .cc files
	out_cc->inc_indent();
	out_cc->println("{");

	GenCleanUpCode(out_cc);	//do nothing
	type_->GenCleanUpCode(out_cc, env_);	//type calls field's clean up, field call type's clean up. eventually do nothing
...
void TypeDecl::GenParsingEnd(Output *out_cc, Env *env, const DataPtr &data)
	...
	if ( type_->incremental_input() )
		...
		if(FLAGS_no_error_detection == false)
			{
...
void TypeDecl::GenParseFunc(Output* out_h, Output* out_cc)
	...
	string proto;
	proto = ParseFuncPrototype(env);	//return proto of function header
	...
	out_h->println(proto.c_str(), "", ";");	//output func header to .h and .cc file
